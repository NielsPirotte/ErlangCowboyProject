...
loop(Locatie, Bestuurder, Naam,Rijd,VN)->
  receive
    ...
    {setBestuurder, NieuweBestuurder, PID} -> 
    	...
    	loop(Locatie, NieuweBestuurder, Naam,Rijd,VN);
    {setLocatie, NieuweLocatie, PID} ->
    	self()!{updateData, PID}, 
    	PID!{log,"setLocatie",NieuweLocatie},
    	loop(NieuweLocatie, Bestuurder,Naam,Rijd,VN);
    {setNaam, NieuweNaam, PID} -> 
    	self()!{updateData, PID},
    	PID!{log,"setNaam" , NieuweNaam}, 
    	loop(Locatie, Bestuurder, NieuweNaam,Rijd,VN);
    {setRijd , NieuweR, PID} -> 
    	PID!{log,"setRijd",[NieuweR,self()]},
      	if(NieuweR == false) ->
      		self()!{verwijderBestuurder,PID},
      		loop(Locatie,Bestuurder,Naam,NieuweR,VN);
        true ->  
        	loop(Locatie,Bestuurder,Naam,NieuweR,VN)
      	end;
    {checkFiets, PIDPers, NieuweLocatie, PID} -> 
    	PIDPers!{verplaatsFiets, self(), NieuweLocatie, PID},
    	loop(Locatie, Bestuurder, Naam,Rijd,VN);
    ...
    {smsSchool} -> 
    	...
    	loop(Locatie, Bestuurder ,Naam,Rijd,VN);
    {smsGearriveerd} -> 
    	io:format("fiets is gearriveerd op ~p met ~p" , [Locatie,Bestuurder]),
    	loop(Locatie, Bestuurder, Naam,Rijd,VN);
    {verwijderBestuurder,PID} -> 
    	self()!{updateData , PID}, 
    	PID!{log,{"verwijderBestuurder"}}, 
    	loop(Locatie , [] ,  Naam,Rijd,VN);
    {getVN,Tijd,PID,PIDP} -> 
